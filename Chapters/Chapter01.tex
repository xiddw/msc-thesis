...
De acuerdo al trabajo desarrollado hasta ahora, se puede distinguir que hay dos grandes enfoques que se usan para \sd: \textit{bottom-up} (de abajo a arriba) y \textit{top-down} (de arriba a abajo). De forma general, se consideran \textit{bottom-up} las metodologías en las que se inicia la estimación con pocos clústers (e incluso un sólo segmento), y \textit{top-down} aquellas en las que se inicia la estimación con muchos más clústers de los que se esperan encontrar.
...
Para ambas estrategias se suelen usar Modelos Ocultos de Markov (HMM por sus siglas en inglés) así como Modelos de Mezclas de Gaussianas (GMM también por sus siglas en inglés) para cada uno de los estados correspondientes, 
